...
| Message line | One line of progress, feedback, an error, or the next action. |
| Rule | A thin line. |
| Key bar | The keys of this screen. |

The message line shows the first of these that applies:
...
| Any other kind | Writes the sentence and `do this in the app.` on the message line. |

A full review needs a model for each role its stages use. With no model, the message line shows the error. [Add a model backend](/how-to/add-a-model-backend/) sets one up.
...
The TUI loads the screen again when the editor closes. When the editor does not start, the message line says `The editor did not open`, with the reason.
