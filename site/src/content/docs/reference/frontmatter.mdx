...
A run that infers the size adds a note: `The doc names no size, so the review used size app. Add "size: app" to the frontmatter to fix it.` When the doc names another value, the note names that value. [Profiles and size](/concepts/profiles-and-size/) says which checks each size runs.
