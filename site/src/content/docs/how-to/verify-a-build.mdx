...
The **History** tab lists the runs, newest first, with **Verified**, **Not Verified** or **Stale**, and the count of each outcome. Click a finished run to see the outcome of each trace ID. **More → Traceability** shows the newest run under **Code and tests**: each trace ID, its outcome, the code target and the test cited.

**Insights** shows the breach rate of each profile beside **False ready**. It is the share of the verified trace IDs that came back breached or missing. A high rate says that the requirements of the profile's docs are built wrong.
...
1. Open the **History** tab, and click the run.

2. Find the missing or breached trace ID, and click **Ask for a waiver**.

3. Write the reason, and click **Ask**.

</Steps>

The outcome row shows the request, the approvals it has, and the approvals the policy needs. After the approval, the row says so.

The CLI and the MCP server have no control that asks for a verification waiver. A script asks through the HTTP API. The doc ID is the last part of the bundle page URL, `/bundles/<bundle id>/docs/<doc id>`:
...
The request waits in the approver's **Inbox** and in the tour, as any waiver does. The inbox link opens the run that the request came from, and the approver clicks **Approve** or **Reject** on the outcome row. [Ask for and approve a waiver](/how-to/ask-for-and-approve-a-waiver/) shows the approval. The next run counts the trace ID as waived, and the run verdict counts the waivers.
