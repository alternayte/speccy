...
A doc with no upstream doc says so once, in its sidecar, and the check passes:
...
- **Referenced**: the SDD names the ID. The detail says how many times.
...
An acknowledgement is about the ID, not a section. An edit to the SDD does not end it. The matrix shows each answer, with its reason.
...
A lint verdict costs nothing, so Speccy lints the SDD again at once, and the new ID is a new gap. A full verdict keeps its model results and goes stale instead, and the control row says "Stale". Run the review again to replace it. Then close the REQ-004 gap as in step 5.
