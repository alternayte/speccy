...
In the Code or Split view, `⌘S` or `Ctrl+S` also saves.
...
The next action now says **Hand it to a builder**. Click it, and the rail opens **History**, which lists the versions and the handoffs of the bundle.

A build packet holds the spec doc, the bundle's assets, the linked spec docs and the trace IDs. It also holds the build questions with their agreed answers. A coding agent takes it with the MCP tool `handoff_bundle`, or you write it to a folder:

```sh
speccy handoff payment-retries --out ../payment-retries-build
```

Speccy records the handoff with the verdict at that moment. The folder holds `HANDOFF.md`, the re-entry prompt. A coding agent reads it to resume the build after it loses its context.
