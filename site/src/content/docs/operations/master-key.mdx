...
A secret that Speccy encrypted with one key does not decrypt with another. Speccy starts with a new key, but each use of an old secret fails with this cause:

```text
the secret does not decrypt with this key; the key file or SPECCY_MASTER_KEY changed
```

You see it in a failed review run, in the result of **Test** on a backend, or in a failed GitHub sync. The reviews, the threads, the waivers and every other record stay readable. Only the three kinds of secret are lost.
...
6. Delete and add again each MCP connection that has a secret. The tools of a connection cannot take a new secret, and **Save tools** needs the old one.
