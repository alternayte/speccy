...
VALUES (sqlc.arg(id), sqlc.arg(workspace_id), sqlc.arg(bundle_id), sqlc.arg(version_id), sqlc.narg(handoff_id),
        sqlc.arg(repo), sqlc.arg(sha), sqlc.arg(base_sha), sqlc.arg(digest), sqlc.arg(verdict), sqlc.arg(counts),
        sqlc.arg(notes), sqlc.arg(started_by), sqlc.arg(created_at));
...
WHERE bundle_id = sqlc.arg(bundle_id) AND repo = sqlc.arg(repo) AND sha <> ''
...
SELECT * FROM verification_run WHERE workspace_id = sqlc.arg(workspace_id) ORDER BY created_at DESC;
