...
VALUES (sqlc.arg(id), sqlc.arg(workspace_id), sqlc.narg(spec_doc_id), sqlc.arg(profile_key), sqlc.arg(anchor_kind),
...
-- name: ListSpecDocThreads :many
SELECT * FROM thread_view WHERE spec_doc_id = sqlc.arg(spec_doc_id) ORDER BY status DESC, last_message_at DESC;
...
SELECT COUNT(*) FROM thread_view WHERE spec_doc_id = sqlc.arg(spec_doc_id) AND blocking AND status = 'open';
...
SELECT m.*, t.spec_doc_id, t.profile_key, t.title
...
VALUES (sqlc.arg(id), sqlc.arg(workspace_id), sqlc.arg(spec_doc_id), sqlc.arg(check_slug), sqlc.arg(level),
...
-- name: ListSpecDocWaivers :many
SELECT * FROM waiver_view WHERE spec_doc_id = sqlc.arg(spec_doc_id) ORDER BY created_at DESC;

-- name: ListVerificationWaivers :many
SELECT * FROM waiver_view
WHERE spec_doc_id = sqlc.arg(spec_doc_id) AND scope = 'verify' AND repo = sqlc.arg(repo) AND status = 'approved';

-- name: ListWorkspaceWaivers :many
SELECT * FROM waiver_view WHERE workspace_id = sqlc.arg(workspace_id) ORDER BY created_at DESC;

-- name: UpsertSpecDocStatusView :exec
...
VALUES (sqlc.arg(spec_doc_id), sqlc.arg(status), sqlc.arg(approvals), sqlc.narg(approved_version),
...
-- name: GetSpecDocStatusView :one
SELECT * FROM spec_doc_status_view WHERE spec_doc_id = sqlc.arg(spec_doc_id);

-- name: ListSpecDocStatusViews :many
SELECT s.* FROM spec_doc_status_view s JOIN spec_doc b ON b.id = s.spec_doc_id WHERE b.workspace_id = sqlc.arg(workspace_id);

-- name: InsertSpecDocReviewer :exec
INSERT INTO spec_doc_reviewer (spec_doc_id, user_id) VALUES (sqlc.arg(spec_doc_id), sqlc.arg(user_id)) ON CONFLICT DO NOTHING;

-- name: ListReviewerSpecDocs :many
SELECT spec_doc_id FROM spec_doc_reviewer WHERE user_id = sqlc.arg(user_id);
...
WHERE f.run_id IN (
    ...
    FROM spec_doc b WHERE b.workspace_id = sqlc.arg(workspace_id) AND b.archived_at IS NULL AND b.profile_key = sqlc.arg(profile_key)
)
...
SELECT * FROM link WHERE workspace_id = sqlc.arg(workspace_id) AND kind = 'supersedes' AND target_spec_doc_id IS NOT NULL;
