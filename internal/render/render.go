// Package render turns markdown into HTML with the review engine's parser (DEC-017). Each block
// carries its source position for overlays and scroll sync.
package render
...
// Links says how relative links in a doc resolve. The zero value leaves them as written.
type Links struct {
	// Image returns the URL of a bundle file used as an image.
	Image func(path string) string
	// MarkFiles marks relative links to bundle files with data-bundle-path, for the editor.
	MarkFiles bool
...
func HTML(src []byte, links Links) (string, error) {
...
// rewriteLinks points relative images at their bundle file, and marks relative links to
// bundle files so the client can open them in the editor.
func rewriteLinks(doc ast.Node, links Links) {
	dir := links.Dir
	...
	_ = ast.Walk(doc, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		switch t := n.(type) {
		case *ast.Image:
			if p, ok := resolve(t.Destination); ok && links.Image != nil {
				t.Destination = []byte(links.Image(p))
			}
		case *ast.Link:
			if p, ok := resolve(t.Destination); ok && links.MarkFiles {
