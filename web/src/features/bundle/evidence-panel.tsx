...
export function EvidencePanel({
  ...
  return (
    <div>
      <h3 className="px-3 pt-3 text-2xs font-semibold tracking-[var(--tracking-caps)] text-ink-3 uppercase">Claims</h3>
      {!runId ? (
        <p className="px-3 py-2 text-xs text-ink-3">Run a full review to check the doc's factual claims.</p>
      ) : claims.isPending ? (
      ...
      <h3 className="mt-2 px-3 pt-3 text-2xs font-semibold tracking-[var(--tracking-caps)] text-ink-3 uppercase">
        Assumptions
      </h3>
          ...
          {assumptions.data.items.map((a) => (
            <li key={a.start}>
              <button
                type="button"
                onClick={() => onOpen(a)}
                className="flex w-full items-start gap-2 px-3 py-2.5 text-left text-sm transition-colors hover:bg-sunken"
              >
